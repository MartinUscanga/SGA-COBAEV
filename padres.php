...
<body class="bg-crema font-sans-clean min-h-screen flex flex-col selection:bg-red-200">
    ...
    <header class="bg-vino px-4 py-3 flex justify-between items-center sticky top-0 z-50 shadow-lg safe-area-top">
        ...
        <div id="panel-avisos" class="hidden absolute top-16 right-4 bg-white rounded-xl shadow-xl border border-zinc-200 overflow-hidden z-[100] w-80 max-h-[70vh]">
            ...
            <div class="border-t border-zinc-200 bg-zinc-50 px-4 py-3">
                <a href="avisos_padres.php" class="flex items-center justify-center space-x-2 w-full px-4 py-2.5 bg-violet-600 hover:bg-violet-700 text-white rounded-lg text-xs font-semibold transition-colors">
    ...
    <script>
        ...
        function renderAvisos(avisos) {
            ...
            const iconosSvg = {
                'institucional': '<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"/></svg>',
                'academico': '<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253"/></svg>',
                'emergencia': '<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4c-.77-.833-1.964-.833-2.732 0L3.34 16.5c-.77.833.192 2.5 1.732 2.5z"/></svg>',
                'pagos': '<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>',
                'cultural': '<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 4V2m0 2a2 2 0 012 2v1a2 2 0 01-2 2 2 2 0 01-2-2V6a2 2 0 012-2zm10 0V2m0 2a2 2 0 012 2v1a2 2 0 01-2 2 2 2 0 01-2-2V6a2 2 0 012-2zM5.5 20H4a1 1 0 01-1-1v-1a3 3 0 013-3h1m13 5h1.5a1 1 0 001-1v-1a3 3 0 00-3-3h-1m-7 5v-5a2 2 0 014 0v5m-4 0h4"/></svg>'
            };
            const coloresCat = {
                'institucional': 'bg-blue-50 text-blue-600',
                'academico': 'bg-emerald-50 text-emerald-600',
                'emergencia': 'bg-red-50 text-red-600',
                'pagos': 'bg-amber-50 text-amber-600',
                'cultural': 'bg-purple-50 text-purple-600'
            };

            let html = '';
            avisos.forEach(aviso => {
                const colorCat = coloresCat[aviso.categoria] || 'bg-zinc-50 text-zinc-600';
                const iconoCat = iconosSvg[aviso.categoria] || iconosSvg['institucional'];
                const noLeido = aviso.leido === false;

                html += '<a href="avisos_detalle.php?id=' + aviso.id + '" class="block px-4 py-3 border-b border-zinc-50 hover:bg-zinc-50 transition-colors">';
                html += '  <div class="flex items-start space-x-3">';
                html += '    <div class="flex-shrink-0 w-8 h-8 rounded-lg ' + colorCat + ' flex items-center justify-center">' + iconoCat + '</div>';
                html += '    <div class="flex-1 min-w-0">';
                html += '      <div class="flex items-center space-x-1.5">';
                if (noLeido) {
                    html += '        <span class="w-1.5 h-1.5 bg-violet-500 rounded-full flex-shrink-0"></span>';
                }
                html += '        <p class="text-xs font-bold text-zinc-700 truncate">' + escapeHtml(aviso.titulo) + '</p>';
                html += '      </div>';
                html += '      <p class="text-[11px] text-zinc-500 mt-0.5 line-clamp-2">' + escapeHtml(aviso.preview) + '</p>';
                html += '      <p class="text-[9px] text-zinc-400 mt-1 uppercase tracking-wider">' + escapeHtml(aviso.fecha_relativa) + '</p>';
                html += '    </div>';
                html += '  </div>';
                html += '</a>';
            });
